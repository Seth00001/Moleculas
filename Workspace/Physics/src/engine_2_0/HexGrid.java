...
//				
//
//				&& normal.x * p.x + normal.y * p.y + normal.z * p.z < 50//105
//				&& normal.x * p.x + normal.y * p.y + normal.z * p.z > 20
				);
	}
...
//	#endregion
	...
	public boolean isInRefillingVolume(int i, int j, int k) {
		...
		return(
					validate(i, j, k)
					&& normal.x * p.x + normal.y * p.y + normal.z * p.z > 30//80 //105
				);	
	}
